...
export class PriorityToken {
    ...
    constructor(level: keyof typeof PRIORITY_LEVELS = 'NORMAL', reason = '', expiresAt: string | null = null) {
    ...
    canPreempt(otherToken: PriorityToken | { value: number } | null): boolean {
        if (this.isExpired()) return false;
        const otherValue = otherToken ? ('value' in otherToken ? otherToken.value : 40) : PRIORITY_LEVELS.NORMAL.value;
...
export class PriorityQueueManager {
     ...
     */
    enqueue(sessionRequest: any, priorityToken?: PriorityToken): string {
...
export class PreemptionManager {
     ...
     */
    canPreempt(newSession: any, existingSession: any): PreemptionResult {
     ...
     */
    findPreemptableSessions(newSession: any, existingSessions: any[], requiredSlots = 1): any[] {
        const preemptable: any[] = [];
        const newPriority = newSession.priorityToken || new PriorityToken('NORMAL');
     ...
     */
    executePreemption(preemptingSessions: any[], preemptedSessions: any[], alternativeSlots: any[]): any {
        const result: any = {
            preempted: [],
            rescheduled: [],
            failed: []
     ...
     */
    findAlternativeSlot(session: any, availableSlots: any[], excludeSlots : any[] = []): any | null {
        ...
        for (const slot of availableSlots) {
            ...
            if (!hasOverlap) {
                return {
                    start: slot.start,
                    end: new Date(new Date(slot.start).getTime() + sessionDuration).toISOString()
                };
            }
        }
 ...
 */
export const applyPriorityHeuristics = (sessions: any[], newRequest: any, availableSlots: any[]): any => {
