...
import { GeneticAlgorithm, DEFAULT_GA_PARAMS, GAParams, GAContext } from './geneticAlgorithm';
import { ParticleSwarmOptimization, DEFAULT_PSO_PARAMS, PSOParams, PSOContext } from './particleSwarmOptimization';
import {
...
export interface SchedulerConfig {
    ga: Partial<GAParams>;
    pso: Partial<PSOParams>;
    timeSlotDurationHours: number;
 ...
 */
export const generateTimeSlots = (config: Partial<Record<string, any>> = {}): TimeSlot[] => {
    const {
        startDate = new Date(),
        horizonDays = 14,
        workingHoursStart = 9,
    ...
    const baseDate = startOfDay(new Date(startDate as Date));
...
export class HybridScheduler {
        ...
        try {
            ...
            if (!eligibility.eligible) {
                const criticalViolations = (eligibility.violations || [])
                    .filter((v: any) => v.severity === SEVERITY.CRITICAL)
                    .map((v: any) => v.message);
     ...
     */
    async retrieveProtocol(therapy: any): Promise<any> {
     ...
     */
    createInitialSchedule(therapy: any, patient: any, resources: any, timeSlots: TimeSlot[], existingSessions: any[]): any[] {
        const sessions: any[] = [];
        ...
        for (const step of workflow) {
            ...
            while (currentSlotIndex < timeSlots.length && !slotFound) {
                ...
                if (!hasConflict) {
                    ...
                    const eligibleTherapists = therapists.filter((t: any) =>
                        !t.skills || t.skills.some((s: string) =>
     ...
     */
    resolveConflicts(newSessions: any[], existingSessions: any[], timeSlots: TimeSlot[], priority: PriorityToken): any {
     ...
     */
    findAlternativeSlot(session: any, timeSlots: TimeSlot[], existingSessions: any[], otherNewSessions: any[]): any {
     ...
     */
    slotsOverlap(slot1: { start: any, end: any }, slot2: { start: any, end: any }): boolean {
     ...
     */
    reportProgress(phase: string, percent: number, message: string) {
        if (this.onProgress) {
            this.onProgress({
                phase,
                progress: percent,
     ...
     */
    getExplainer(): ExplainabilityEngine {
 ...
 */
export const createScheduler = (config: Partial<SchedulerConfig> = {}): HybridScheduler => {
