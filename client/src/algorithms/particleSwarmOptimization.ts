...
export class Particle {
    ...
    constructor(position: number[], velocity: number[] | null = null) {
...
/**
 * PSO context interface
 */
export interface PSOContext {
    timeSlots: any[];
    therapists: any[];
    rooms: any[];
...
export class ParticleSwarmOptimization {
    ...
    context: PSOContext | null;
    sessions: any[] | null;

    constructor(params: Partial<PSOParams> = {}) {
     ...
     */
    initializeSwarm(sessions: any[], context: PSOContext) {
     ...
     */
    calculateFitness(position: number[]): number {
        if (!this.context || !this.sessions) return 0;
     ...
     */
    countRoomConflicts(sessions: any[]): number {
        let conflicts = 0;
     ...
     */
    countTherapistConflicts(sessions: any[]): number {
        let conflicts = 0;
        for (let i = 0; i < sessions.length; i++) {
            for (let j = i + 1; j < sessions.length; j++) {
     ...
     */
    countExistingConflicts(newSessions: any[], existingSessions: any[]): number {
        let conflicts = 0;
     ...
     */
    calculateSequenceScore(position: number[]): number {
        let score = 0;
     ...
     */
    calculateCompactness(position: number[]): number {
     ...
     */
    calculateUtilization(sessions: any[], rooms: any[]): number {
     ...
     */
    doIntervalsOverlap(interval1: { start: any, end: any }, interval2: { start: any, end: any }): boolean {
    ...
    updateParticle(particle: Particle) {
        if (!this.globalBest || !this.context) return;
     ...
     */
    run(sessions: any[], context: PSOContext): any {
     ...
     */
    applyBestSolution(): any[] {
        if (!this.globalBest || !this.sessions || !this.context) {
