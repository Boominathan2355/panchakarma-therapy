...
import { validateAllConstraints } from './ruleBasedConstraints';

/**
 * GA parameters interface
 */
...
export class Chromosome {
    ...
    constructor(genes: Gene[] = [], fitness: number = 0) {
        this.genes = genes;
...
export class GeneticAlgorithm {
     ...
     */
    initializePopulation(context: GAContext) {
     ...
     */
    createRandomChromosome(context: GAContext): Chromosome {
     ...
     */
    calculateFitness(chromosome: Chromosome, context: GAContext): number {
        ...
        fitness -= (validation.summary.critical || 0) * 25;
        fitness -= (validation.summary.warnings || 0) * 5;
        ...
        for (const gene of chromosome.genes) {
            ...
            if (therapist?.skills?.some((s: string) =>
                s.toLowerCase().includes(therapy.name.toLowerCase())
            )) {
                skillMatchBonus += 5;
            }
    ...
    mutate(chromosome: Chromosome): Chromosome {
        if (!this.context) return chromosome;
    ...
    evolveGeneration(): any {
        ...
        if (this.population[0].fitness > (this.bestChromosome?.fitness || 0)) {
            this.bestChromosome = this.population[0].clone();
        }
