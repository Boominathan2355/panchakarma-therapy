    ...
    const utilizationData = useMemo(() => {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            const totalHoursBoxed = daySessions.reduce((acc, s) => {
                const duration = (new Date(s.end).getTime() - new Date(s.start).getTime()) / (1000 * 60 * 60);
                return acc + duration;
            }, 0);
            ...
            const activeTherapists = new Set(daySessions.map((s: any) => s.therapistId)).size;
        ...
        <div className="resource-page">
            ...
            <div className="resource-content">
                {(isLoading || isTransitioning) ? (
                    ...
                    <>
                        {activeTab === 'staff' && <StaffDirectory staff={staff as any} />}

                        {activeTab === 'inventory' && (
                            <div className="limit-width">
                                <MaterialInventory items={inventory as any} />
                            </div>
                        )}
