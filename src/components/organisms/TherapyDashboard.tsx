...
export interface TherapyDashboardProps {
    therapies: Therapy[];
...
const TherapyDashboard: React.FC<TherapyDashboardProps> = ({ 
    therapies, 
    onSelect, 
    isLoading = false, 
    searchTerm, 
    ...
    const getRelevance = (therapy: Therapy, term: string): number => {
        if (!term) return 0;
    ...
    const filteredTherapies = therapies
        ...
        .filter(t => searchTerm === '' || (t.score || 0) > 0)
        .sort((a, b) => (b.score || 0) - (a.score || 0));

    const getTherapyColor = (index: number) => {
        const colors = [
                ...
                <div className="therapy-cards-grid">
                    {[...Array(6)].map((_, i) => (
                        <div key={i} className="therapy-card skeleton-card">
                            <Skeleton width="100%" height="70px" className="rounded-t-lg" />
                            <div className="p-4">
                                <Skeleton width="60%" height="1rem" className="mb-2" />
                                <Skeleton width="90%" height="0.75rem" className="mb-1" />
