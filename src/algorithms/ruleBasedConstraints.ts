...
import type { Patient, Therapist, Material, ScheduleEntry } from '../types';
...
export interface ValidationResult {
    ...
    eligible?: boolean; // For patient eligibility
}

/**
 * Validates that therapy workflow steps are in correct order
 */
export const validateTherapySequence = (sessions: any[], therapy: any): ValidationResult => {
    ...
    for (const session of sortedSessions) {
        const workflowStep = therapy.workflow.find((w: any) => w.action === session.action);
        if (workflowStep) {
 ...
 */
export const validateSessionGaps = (sessions: any[], gapConstraints = { minHours: 1, maxHours: 48 }): ValidationResult => {
    const violations: ConstraintViolation[] = [];
    const sortedSessions = [...sessions].sort((a, b) =>
        new Date(a.start).getTime() - new Date(b.start).getTime()
    );
    ...
    for (let i = 1; i < sortedSessions.length; i++) {
        const prevEnd = new Date(sortedSessions[i - 1].end).getTime();
        const currStart = new Date(sortedSessions[i].start).getTime();
 ...
 */
export const checkPatientContraindications = (patient: Patient, therapy: any): ValidationResult & { eligible: boolean } => {
    const violations: ConstraintViolation[] = [];

    if (!therapy.contraindications || !patient.conditions) {
        return { eligible: true, valid: true, violations: [] };
    }
    ...
    if (patient.age) {
        const ageContraindications = (therapy.contraindications as string[]).filter(c =>
 ...
 */
export const validateTherapistAvailability = (sessions: any[], therapists: Therapist[]): ValidationResult => {
    const violations: ConstraintViolation[] = [];
 ...
 */
export const validateRoomAvailability = (sessions: any[], existingSessions: any[] = []): ValidationResult => {
    const violations: ConstraintViolation[] = [];
 ...
 */
export const validateMaterialSufficiency = (sessions: any[], inventory: Material[], requirements: any = {}): ValidationResult => {
    const violations: ConstraintViolation[] = [];

    // Default material requirements
    const defaultRequirements: any = {
 ...
 */
export const validatePatientAvailability = (sessions: any[], patient: Patient): ValidationResult => {
    const violations: ConstraintViolation[] = [];
 ...
 */
function doIntervalsOverlap(interval1: { start: Date; end: Date }, interval2: { start: Date; end: Date }): boolean {
